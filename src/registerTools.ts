...
const registerTools = (server: McpServer) => {
      ...
      try {
        ...
        return {
          content: [
            {
              type: 'text',
              text: `Error: ${e.message}${e.code ? ` (${e.code})` : ''}`,
            },
          ],
          isError: true,
        };
  ...
  server.tool(
    ...
    'Investigates and creates a strategic plan to accomplish a task',
    {
      args: z
        .string()
        .describe('Task or objective to create a strategic plan for'),
      workingDir: z
        .string()
        .optional()
        .describe('Directory to run the command in'),
  ...
  server.tool(
    ...
    'Analyzes and creates comprehensive documentation strategies for any subject or project',
    {
      args: z
        .string()
        .describe('Subject or project to create documentation strategy for'),
      workingDir: z
        .string()
        .optional()
        .describe('Directory to run the command in'),
  ...
  server.tool(
    ...
    'Analyzes system architecture and designs solutions at the system level',
    {
      args: z
        .string()
        .describe('System requirements or design challenge to analyze'),
      workingDir: z
        .string()
        .optional()
        .describe('Directory to run the command in'),
